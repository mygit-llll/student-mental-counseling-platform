...
public class JwtUtil {
  ...
  public String sign(
    ...
    // 函数式创建 token，避免重复书写
    final Supplier<String> createToken = () -> this.createToken(accountId,name, grantedAuthorities, isAdmin);
    // 看看缓存有没有账户 token
    ...
    final Object redisTokenValidateObj = this.redisUtils.getValue(token);
    final boolean isValidate = redisTokenValidateObj instanceof Boolean ? (Boolean) redisTokenValidateObj : true;
    // token 仍有效
    if (isValidate) {
      // 删除，重新签发
  ...
  public void invalidRedisToken(final String name) {
    ...
    Optional.ofNullable(token).ifPresent(_token -> {
      this.redisUtils.setValue(_token, false); // 使旧 token 失效
      this.redisUtils.delete(name); // 删除 username → token 映射
    });
  }
  ...
  public boolean validateToken(final String token) {
    ...
    if (redisTokenValidate != null) {
      ...
      isValidate = redisTokenValidate instanceof Boolean ? (Boolean) redisTokenValidate : true;
    }
  ...
  /**
   * 获取 Token 的 Claims（payload），用于内部工具类解析自定义字段如 accountId
   */
  public Claims getClaimsFromToken(String token) {
    ...
    // 去掉 Bearer 前缀（如果存在）
    String cleanToken = token.startsWith(this.jwtProperties.getTokenType())
            ? token.substring(this.jwtProperties.getTokenType().length()).trim()
            : token;
